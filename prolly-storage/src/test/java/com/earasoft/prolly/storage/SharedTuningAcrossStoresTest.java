 ...
 *
 * <p><b>What these tests do and do not discriminate.</b> Sharing itself is proven: giving each
 * store its own equally-sized tuning turns {@code
 * storesShareONECacheRatherThanOneEachOfTheSameSize} red (usage 88 -> 88, no movement). CAPACITY
 * alone would not have — two stores each holding their own 64 MiB cache report exactly what two
 * sharing one report, which is why an earlier version of this test was vacuous. The {@code
 * ownsTuning} guard in {@code RocksNodeStore.closeNative}, by contrast, is <b>defensive and not
 * discriminated here</b>: making a shared-tuning store free the handles anyway leaves all three
        ...
        withCacheProperty(
                () -> {
                    try (RocksTuning shared = RocksTuning.fromSystemProperties();
                            RocksNodeStore a =
                                    new RocksNodeStore(dir.resolve("a").toString(), shared);
                            RocksNodeStore b =
                                    new RocksNodeStore(dir.resolve("b").toString(), shared)) {

                        // CAPACITY cannot prove sharing: two stores each with their own 64 MiB
                        // cache report
                        // exactly what two stores sharing one report. USAGE can — a shared cache is
                        // one
                        ...
                        assertTrue(
                                after > before,
                                "store B's cache usage did not move when store A read ("
                                        + before
                                        + " -> "
                                        + after
                                        + ") — the stores are NOT sharing one cache");
                    } catch (Exception e) {
