 ...
 *
 *
 * <h3>BuzHash Random Table</h3>
 *
 * <p>Contains the 256-entry random uint32 table used by {@link BuzHash}. <b>Critical Note:</b>
 *
 * <p>These values are taken from the kch42/buzhash reference. Changing even a single bit shifts
 * every chunk boundary — which breaks the port's <b>own</b> internal determinism, its pinned
 * goldens ({@code ChunkerDeterminismGateTest}), and any existing port database's format. It is
 * <b>not</b> a Dolt-compat constraint: the port's chunker already diverges from Dolt v2.0.3 ({@code
 * cross-lang/BITCOMPAT_FINDINGS.md}) and byte-for-byte Dolt parity is optional/deferred.
 */
public class BuzHashTable {
    public static final int[] DEFAULT_TABLE = {
